...
#    camera.start_preview(fullscreen=False, window=(0, 50, CANVAS_W+50, CANVAS_H), hflip=True, vflip=False)
    can.delete("image")
    led_state = False
...
#    camera.preview_window = (0, 0, SCREEN_W, SCREEN_H)
    ...
    for i in range(countdown1):
        can.delete("text")
        can.update()
        can.create_text(int(CANVAS_W/2), int(CANVAS_H/2), text=str(countdown1 - i), font=font, tags="text")
        can.update()
        if i < countdown1 - 2:
            time.sleep(1)
